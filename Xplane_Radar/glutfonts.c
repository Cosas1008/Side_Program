...
draw_stuff()
{
  ...
  const char* bitmap_font_names[7] = {
   ...
   if (mode == MODE_BITMAP) {
      ...
      print_bitmap_string(bitmap_fonts[font_index], (char *) bitmap_font_names[font_index]); // add char * cast here
      for (j=0; j<7; j++) {
         glRasterPos2f(x, y);
      ...
      glPushMatrix(); {
         ...
         print_stroke_string(
            stroke_fonts[font_index],(char *) stroke_font_names[font_index]); // add char * cast here
      } glPopMatrix();
      glPushMatrix(); {
